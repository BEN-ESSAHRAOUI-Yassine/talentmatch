<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Modifier l\'offre') }} : {{ $offre->title }}
        </h2>
    </x-slot>
                ...
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('offres.update', $offre) }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="title" name="title" type="text" class="mt-1 block w-full" :value="old('title', $offre->title)" required />
                            <x-input-error :messages="$errors->get('title')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="description" value="Description" />
                            <textarea id="description" name="description" rows="6" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>{{ old('description', $offre->description) }}</textarea>
                        ...
                        <div>
                            ...
                            <x-text-input id="minimum_experience" name="minimum_experience" type="number" min="0" class="mt-1 block w-48" :value="old('minimum_experience', $offre->minimum_experience)" required />
                            <x-input-error :messages="$errors->get('minimum_experience')" class="mt-2" />
                        </div>

                        <div class="flex items-center gap-4">
                            <x-primary-button>{{ __('Mettre à jour') }}</x-primary-button>
                            <a href="{{ route('offres.show', $offre) }}" class="text-sm text-gray-600 hover:underline">Annuler</a>
                        </div>
                    </form>
                </div>
            </div>
