...
Allow authenticated users to submit a candidate CV (name + text) against one of their job offers (offres). A candidat record is created, an associated analyse with `status=pending` is created, and an `AnalyseCvJob` is queued for AI processing.
