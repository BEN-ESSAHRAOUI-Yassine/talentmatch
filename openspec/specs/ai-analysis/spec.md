...
When a candidate is submitted, the system SHALL dispatch a queued `AnalyseCvJob` that uses the `laravel/ai` structured output agent to extract and evaluate candidate information against the job offer requirements.
